...
1. **Log in** to [auth.sendexa.co](https://auth.sendexa.co)
2. Navigate to **SMPP → Settings → Webhooks**
3. Click **Add Webhook**
4. Configure:
...
1. Go to **SMPP → Settings → Webhooks**
2. Select webhook
...
1. Navigate to **SMPP → Webhooks → Logs**
2. See:
   - Delivery attempts
   - Response codes
   - Response times
...
```
SMPP DLR:     Send ──▶ Network ──▶ DLR (100ms)
HTTP Webhook: Send ──▶ Network ──▶ Queue ──▶ HTTP (500ms)
```
...
- 📊 **[Delivery Reports](/docs/smpp/dlr)** - Native SMPP DLRs
- 🛠️ **[API Reference](/docs/smpp/v1)** - Complete specification
- 📤 **[Send SMS](/docs/smpp/send)** - Submit messages
